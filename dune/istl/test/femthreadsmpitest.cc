...
inline double uExact(const double& x)
{
  return x*x+1;
}
...
void assemble(std::size_t tid,M& A_thr,V& b_thr,G& grid,size_t numGridElements,F& phi,F& derphi)
{
  std::vector<std::vector<double>> A_entity(2,std::vector<double>(2,0.0));
  std::vector<double> b_entity(2,0.0);
  const std::size_t startElem(tid*numGridElements);
  const std::size_t endElem((tid+1)*numGridElements);
...
{
  const std::size_t dim(b_thr.size());
  const std::size_t offset((dim-1)*tid);
  for(std::size_t i=0;i!=dim;++i)
  {
    b[i+offset]+=b_thr[i];
    for(std::size_t j=0;j!=dim;++j)
      A[i+offset][j+offset]+=A_thr[i][j];
  }
...
public:
  ...
  static IndexedType gather(const T& v,int i)
  {
    return v[i];
  }
  static void scatter(T& v,IndexedType item,int i)
  {
    v[i]=item;
  }
};
...
public:
  ...
  static IndexedType gather(const T& m,int i)
  {
    return m[i/m.rows()][i%m.rows()];
  }
  static void scatter(T& m,IndexedType item,int i)
  {
    m[i/m.rows()][i%m.rows()]=item;
  }
};
...
int main(int argc,char** argv)
{
  ...
  const std::size_t numNodesGlobal((numNodes-1)*size+1);
  ...
  typedef Dune::ParallelIndexSet<std::size_t,LocalIndexType,numNodes> VectorParallelIndexType;
  typedef Dune::ParallelIndexSet<std::size_t,LocalIndexType,numNodes*numNodes> MatrixParallelIndexType;

  // create parallel index set for vector
  VectorParallelIndexType vectorPIS;
  const std::size_t vectorFirstGlobalIdx((numNodes-1)*rank);
  const std::size_t vectorLastGlobalIdx(vectorFirstGlobalIdx+numNodes-1);
  flags flg;
  ...
  for(std::size_t i=(vectorFirstGlobalIdx+1);i!=(vectorLastGlobalIdx-1);++i)
    vectorPIS.add(i,LocalIndexType(i-vectorFirstGlobalIdx,owner));
  if(rank==(size-1))
    flg=border;
  else
    flg=overlap;
  ...
  const std::size_t matrixGlobalOffset((numNodes-1)*rank);
  ...
  std::size_t numColors(2); // 0 when tid is even, 1 when tid is odd
  ...
  for(std::size_t i=0;i!=numThreads;++i)
    thr[i]=std::thread(assemble<StiffnessMatrixType,VectorType,GridType,std::vector<FunctionType>>,i,std::ref(A_thr[i]),std::ref(b_thr[i]),std::ref(grid),numGridElementsPerThread,std::ref(phi),std::ref(derphi));
  for(std::size_t i=0;i!=numThreads;++i)
    thr[i].join();
  ...
  {
    for(std::size_t j=0;j!=colors[i].size();++j)
      thr[j]=std::thread(push<StiffnessMatrixType,VectorType>,colors[i][j],std::ref(A_thr[i]),std::ref(b_thr[i]),std::ref(A),std::ref(b));
    for(std::size_t j=0;j!=colors[i].size();++j)
      thr[j].join();
  }
